...
void TestVectors(int ver)
{
	...
	for(n = 1; n < 50; n++)
	{
        ...
        fprintf(fic, "%d\n", n);
		for(i = 0; i < (versions[ver][1]>>3); i++) fprintf(fic,"%02x", k[i]); fprintf(fic,"\n");
		for(i = 0; i < (versions[ver][0]>>3); i++) fprintf(fic,"%02x", p[i]); fprintf(fic,"\n");
		enc(c,k,ver);
		for(i = 0; i < (versions[ver][0]>>3); i++) fprintf(fic,"%02x", c[i]); fprintf(fic,"\n");
	}
}
